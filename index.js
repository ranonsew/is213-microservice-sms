...
router.route('/')
    ...
    if (!message || typeof message != "string" || !receiver || typeof receiver != "string")
        res.status(400).json(return_msg("One or more body variables has not been added"));
    if (!TWILIO_ACCOUNT_SID || !TWILIO_AUTH_TOKEN || !TWILIO_PHONE_NUMBER)
        res.status(500).json(return_msg("One or more environment variables are missing"));
    else {
